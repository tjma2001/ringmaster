...
class DbManager {
  constructor() {
    db.serialize(() => {
      ...
      db.run(`
          CREATE TABLE IF NOT EXISTS 
            nodestats (
              pubkey TEXT PRIMARY KEY UNIQUE,
              stats TEXT
            )`);
    });
  }
  ...
  async getNodeStats(pubKey) {
    const query = `SELECT * FROM nodestats where pubkey ='${pubKey}'`;
    console.log('query', query);
    const result = await asyncGet(query);
    return result ? result[0] : null;
  }

  async updateNodeStats(pubkey, stats) {
    console.log(pubkey);
    const query = `
